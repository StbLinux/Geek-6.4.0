...
using Geek.AbpGeek.Tests.TestDatas;
...
namespace Geek.AbpGeek.Tests
{
    ...
    /// Provides methods to easily work with <see cref="AbpGeekDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<AbpGeekTestModule>
    {
        ...
        {
            void NormalizeDbContext(AbpGeekDbContext context)
            {
        ...
        protected void UsingDbContext(Action<AbpGeekDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AbpGeekDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AbpGeekDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AbpGeekDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AbpGeekDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpGeekDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AbpGeekDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpGeekDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AbpGeekDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AbpGeekDbContext, Task<T>> func)
        {
            T result;
